...
public final class ScreencastDispatcher {
    ...
    private class BitmapFetchRunnable implements Runnable {
        ...
        private void updateScreenBitmap() {
            Activity activityTryGetTopActivity;
            if (ScreencastDispatcher.this.mIsRunning && (activityTryGetTopActivity = ScreencastDispatcher.this.mActivityTracker.tryGetTopActivity()) != null) {
                View decorView = activityTryGetTopActivity.getWindow().getDecorView();
                try {
                    if (ScreencastDispatcher.this.mBitmap == null) {
                        float width = decorView.getWidth();
                        float height = decorView.getHeight();
                        float fMin = Math.min(ScreencastDispatcher.this.mRequest.maxWidth / width, ScreencastDispatcher.this.mRequest.maxHeight / height);
                        int i10 = (int) (width * fMin);
                        int i11 = (int) (fMin * height);
                        ScreencastDispatcher.this.mBitmap = Bitmap.createBitmap(i10, i11, Bitmap.Config.RGB_565);
                        ...
                        ScreencastDispatcher.this.mTempDst.set(0.0f, 0.0f, i10, i11);

...
public enum f implements ui.c {
    ...
    IIRV_EXCEEDS_MAX_VECTORS("cannot embed more than 1000 vectors in a single IIRV message file: got {0}. Consider splitting the data into multiple files"),
    ...
    IIRV_SEQUENCE_NUMBER_MUST_BE_INCREASING_BY_ONE("IIRV sequence number must increase by 1 for each subsequent vector entry. At at line {0}, expected sequence number {1}+1, but found {2}"),
    ...
    public String m(Locale locale) {
        return a("assets/org/orekit/localization/OrekitMessages", name(), locale);
    }
}
